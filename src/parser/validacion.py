...
def _validar_minimos(tipo: str, visita: dict, errores: list[str]) -> None:
    """Comprueba tipo de registro y campos mínimos de visita."""
    if not tipo:
        errores.append("TIPO_REGISTRO ausente")
        return
    for campo in MINIMOS_VISITA.get(tipo, []):
        if _falta(visita, campo):
            errores.append(f"{campo.upper()} ausente en visita")


def _validar_bloques_tipo(tipo: str, registro: dict, errores: list[str]) -> None:
    ...
    if tipo == "FIN_VISITA_LINDUS":
        _validar_lista_minima(meteo, ["hora_meteo"], "meteorologia", errores, permite_vacia=True)
...
def _validar_lista_minima(lista: list, campos: list[str], nombre: str, errores: list[str], permite_vacia=False) -> None:
    """Valida una lista de bloques con campos obligatorios."""
    if not lista and not permite_vacia:
        errores.append(f"Al menos un bloque {nombre} requerido")
    for indice, bloque in enumerate(lista, start=1):
        for campo in campos:
            if _falta(bloque, campo):
                errores.append(f"{campo.upper()} ausente en {nombre} {indice}")


def _validar_unico(lista: list, campos: list[str], nombre: str, errores: list[str]) -> None:
    """Valida que exista exactamente un bloque con mínimos."""
    if len(lista) != 1:
        errores.append(f"Debe haber exactamente un bloque {nombre}")
        return
    for campo in campos:
        if _falta(lista[0], campo):
            errores.append(f"{campo.upper()} ausente en {nombre} 1")


def _validar_cebo(datos: list, errores: list[str]) -> None:
    """Valida el bloque único de cebo de avispón."""
    capturas = {"vv", "crabro", "avispa_europea", "polilla", "mariposa", "otros"}
    if len(datos) != 1:
        errores.append("Debe haber exactamente un bloque cebo_avispon")
        return
    if not capturas.intersection(datos[0]) and _falta(datos[0], "observaciones_cebo"):
        errores.append("Falta al menos una captura u OBSERVACIONES_CEBO en cebo_avispon 1")


def _validar_formatos(registro: dict, errores: list[str]) -> None:
...
def _validar_nubosidad(contexto: str, bloque: dict, errores: list[str]) -> None:
    """Comprueba que nubosidad esté entre 0 y 8."""
    if "nubosidad" in bloque and not 0 <= bloque["nubosidad"] <= 8:
        errores.append(f"NUBOSIDAD inválida en {contexto}: {bloque['nubosidad']}")


def _validar_cerrado(contexto: str, bloque: dict, errores: list[str]) -> None:
    ...
    for campo, validos in reglas.items():
        if campo in bloque and bloque[campo] not in validos:
            errores.append(f"{campo.upper()} inválido en {contexto}: {bloque[campo]}")
