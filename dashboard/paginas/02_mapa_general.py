...
def _render_filtros(tablas: dict[str, pd.DataFrame]) -> dict[str, object]:
    ...
    with st.container(border=True):
        ...
        desde, hasta = _selector_fechas(tablas.get("visitas", pd.DataFrame()))
...
def _selector_fechas(visitas: pd.DataFrame) -> tuple[date | None, date | None]:
    ...
    if fechas.empty:
        return None, None
    rango = st.date_input("Rango de fechas", value=(fechas.min(), fechas.max()))
